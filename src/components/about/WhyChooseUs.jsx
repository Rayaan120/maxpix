...
const WhyChooseUs = () => {
    ...
    useLayoutEffect(() => {
        const checkMobile = () => setIsMobile(window.innerWidth < 1024); // Disable complex effects on tablet too
        checkMobile();
    ...
    return (
        <section ref={containerRef} className="py-10 md:py-14 bg-[#f2f0eb] relative overflow-hidden">
            {/* Abstract Parallax Elements - Higher contrast for premium feel - Disabled on Mobile */}
            {!isMobile && (
                <>
                ...
                <div className="flex flex-col lg:flex-row items-center gap-10 lg:gap-16">
                    ...
                    <div className="lg:w-1/2 w-full">
                        ...
                        >
                            <h2 className="text-5xl md:text-8xl font-black text-[#0a0a0a] leading-[0.85] tracking-tighter mb-6">
                                WHY <br />
                                <span
                                    className={`block ${isMobile ? 'text-[var(--color-primary-red)]' : 'text-outline-dark'}`}
                                    style={isMobile ? {} : {
                            ...
                            <div className="grid grid-cols-1 sm:grid-cols-2 gap-10">
                                <div className={`p-8 bg-white rounded-3xl border border-gray-100 ${isMobile ? '' : 'backdrop-blur-sm shadow-sm hover:shadow-md transition-shadow'}`}>
                    ...
                    >
                        ...
                        <motion.div
                            ...
                            className={`absolute -bottom-8 left-8 right-8 md:right-auto md:-left-16 md:bottom-12 z-20 bg-white border border-white p-8 rounded-[2rem] md:max-w-xs will-change-transform transform-gpu ${isMobile ? '' : 'backdrop-blur-2xl shadow-[0_20px_50px_rgba(0,0,0,0.1)]'}`}
                        >
